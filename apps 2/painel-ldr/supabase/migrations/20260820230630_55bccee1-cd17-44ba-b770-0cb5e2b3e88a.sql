CREATE OR REPLACE FUNCTION private.site_order_map_payment(_v text)
...
AS $$
  SELECT CASE lower(coalesce(_v,''))
    ...
    WHEN 'partially_refunded' THEN 'parcialmente_reembolsado'
    WHEN 'parcialmente_reembolsado' THEN 'parcialmente_reembolsado'
...
CREATE OR REPLACE FUNCTION private.site_order_map_status(_v text)
RETURNS public.order_status
LANGUAGE sql IMMUTABLE SET search_path TO 'public'
AS $$
  SELECT CASE lower(coalesce(_v,''))
    WHEN 'payment_confirmed' THEN 'em_andamento'
    WHEN 'refunded' THEN 'cancelado'
    WHEN 'partially_refunded' THEN 'em_andamento'
    WHEN 'novo' THEN 'novo'
...
CREATE OR REPLACE FUNCTION private.recruitment_unit(_key text)
RETURNS TABLE (currency text, unit_cents integer)
LANGUAGE sql IMMUTABLE SET search_path TO 'public'
AS $$
  SELECT t.c::text, t.u::integer FROM (VALUES
    ('recruitment_operational_eu','EUR',40500),
    ('recruitment_specialized_eu','EUR',81000),
    ('recruitment_operational_br','BRL',216000),
    ('recruitment_specialized_br','BRL',432000)
  ) AS t(k, c, u)
  WHERE t.k = _key
$$;
...
BEGIN
  ...
  IF _order_id IS NULL THEN
    INSERT INTO public.orders (
      ...
      coalesce(_title,'Solicitacao do site'), _key, _category, _final_curr, _final_amount, _qty,
  ...
  ELSE
    UPDATE public.orders
       SET customer_id = coalesce(customer_id, _customer_id),
           ...
           catalog_key = coalesce(_key, catalog_key),
...
CREATE OR REPLACE FUNCTION public.process_site_order(_source text, _payload jsonb)
RETURNS jsonb
LANGUAGE sql SET search_path TO 'public', 'private'
AS $$ SELECT private.process_site_order(_source, _payload) $$;

REVOKE ALL ON FUNCTION private.process_site_order(text, jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.site_order_map_payment(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.site_order_map_status(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.recruitment_unit(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.process_site_order(text, jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.process_site_order(text, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.process_site_order(text, jsonb) TO service_role;